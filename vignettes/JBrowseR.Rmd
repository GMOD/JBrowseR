...
holds, written as R lists. There are no constructors to learn, so a track type
or view type JBrowse gains needs nothing added to the package.
...
**A custom theme.** The `theme` config recolors the browser; it is the
...
- Keeping a browser in a JSON file? Pass it as
  [`config`](json-tutorial.html).
- Embedding in Shiny? Use `JBrowseROutput()` and `renderJBrowseR()`, read the
  ...
  with `update_location("<outputId>", loc)`.
