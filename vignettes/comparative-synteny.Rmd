...
is `list(type = , ...)` with the view's settings written beside `type` — the
same object a `config.json`'s `defaultSession.views` holds, and the same one
JBrowse Web reads from its
[`?session=spec-…` URLs](https://jbrowse.org/jb2/docs/urlparams/). The settings
are the view's own
...
A single `MultiGenomePAFAdapter` track serves every pair from one PAF file. It
spans all four assemblies. The track config is a plain list, the same JSON a JBrowse
...
The view's own `views` lists the panels top-to-bottom, one `list(assembly = )` each. The
...
Shiny, pair `JBrowseRAppOutput()` with `renderJBrowseRApp()`.
